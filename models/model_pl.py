...
class TransformerModelPL(LightningModule):
    ...
    def training_step(
        ...
        mse_bbox_loss = (
            self.point_criterion(pred_bboxes, gt_bboxes).sum(dim=1) * class0_mask
        )
    ...
    def validation_step(
        ...
        gt_visibilities = gt_visibilities.view(-1, ModelConfig.num_keypoints)
        gt_visibilities = gt_visibilities[target_indices]
        # create class0 mask to be used in loss computation of bboxes and keypoints
        ...
        class_accuracy_wo0 = self.accuracy(
            pred_classes[other_class_indices].argmax(dim=1),
            gt_classes[other_class_indices],
        )
        bbox_giou_w0 = generalized_box_iou_loss(
            pred_bboxes,
            gt_bboxes,
            reduction="none",
        )
        bbox_giou_wo0 = generalized_box_iou_loss(
